                  ...
                  <ul className="list-group list-group-flush" id="session-info">
                    ...
                    {session.discountTier &&
                      <li className="list-group-item">
                        Discounted price: up to ${maxDiscountTier.price} if more than {maxDiscountTier.qty} players purchase.
                      </li>
                    }
                    {session.agenda &&
                      <li className="list-group-item">Time: {
                        `${(new Date(session.agenda.start)).toLocaleDateString()}:
                        ${(new Date(session.agenda.start)).toLocaleTimeString('en-US', { timeZone: 'UTC', hour12: true })}`
                      }
                      </li>}
